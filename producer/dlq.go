...
// ProducerDLQEntry records a single order that failed to publish after retries.
type ProducerDLQEntry struct {
...
// fileDLQ appends ProducerDLQEntry records to a JSONL file on disk. It is safe
...
func (d *fileDLQ) Append(entry ProducerDLQEntry) error {
	d.mu.Lock()
	defer d.mu.Unlock()
